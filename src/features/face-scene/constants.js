export const DRACO_PATH = "/draco/";
export const FACE_MODEL_URL = "/models/face/fractured-face.glb";
export const PAGE_BACKGROUND = "#f2f2f2";

export const MODEL_HEIGHT = 1.7;
export const MODEL_VISUAL_OFFSET = [0, 0, 0];
export const MODEL_RESPONSIVE_SCALE_BREAKPOINT = 800;
export const MODEL_RESPONSIVE_SCALE = 0.9;
export const MODEL_RESPONSIVE_X_OFFSET = -0.64;
export const MODEL_RESPONSIVE_Y_OFFSET = -0.7;
export const HOVER_RADIUS = 0.8;
export const HOVER_CORE = 0.4;
export const POINTER_EASE = 5;
export const FRAGMENT_EASE_IN = 8;
export const FRAGMENT_EASE_OUT = 7;
export const DRAG_ROTATION_EASE = 14;
export const DRAG_SPIN_SPEED = 0.006;
export const DRAG_TILT_SPEED = 0.0014;
export const DRAG_MAX_TILT = 0.2;
export const INTERACTION_PLANE_WIDTH = 2.2;
export const INTERACTION_PLANE_HEIGHT = 3.25;
export const INTERACTION_PLANE_Y = -0.08;
export const INTERACTION_PLANE_Z = 1.04;

export const DEFAULT_SCENE_SETTINGS = {
  showModel: true,
  showFps: false,
  modelPositionX: 0.66,
  modelPositionY: 0.46,
  modelPositionZ: 0,
  modelRotationX: -9,
  modelRotationY: -23.9,
  modelRotationZ: -6,
  ambientLightColor: "#ffffff",
  ambientLightIntensity: 0,
  keyLightColor: "#ffffff",
  keyLightIntensity: 1.85,
  keyLightX: -3.1,
  keyLightY: -8,
  keyLightZ: 2.6,
  verticalLightColor: "#fffafa",
  verticalLightIntensity: 1.8,
  verticalLightX: 4.85,
  verticalLightY: -8,
  verticalLightZ: -0.55,
  fillLightColor: "#ffffff",
  fillLightIntensity: 1.5,
  fillLightX: -2.95,
  fillLightY: -1.8,
  fillLightZ: 2.95,
  pointLightColor: "#252d2c",
  pointLightIntensity: 3.2,
  pointLightX: 3.05,
  pointLightY: 1.3,
  pointLightZ: 2.3,
  spotLightColor: "#ffffff",
  spotLightIntensity: 6.2,
  spotLightX: -0.45,
  spotLightY: 2.85,
  spotLightZ: 2.05,
  spotLightAngle: 0.53,
  spotLightPenumbra: 0,
};
